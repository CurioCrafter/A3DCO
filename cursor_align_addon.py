    ...
    try:
        ...
        if cursor_pie_class and hasattr(cursor_pie_class, 'draw'):
            ...
            def enhanced_draw(self, context):
                # Call original Machine Tools draw method first
                self._original_draw_cursor_align(context)
                
                # Add our cursor alignment tools positioned above in the pie menu
                layout = self.layout
                pie = layout.menu_pie()
                
                # Position our tools in the TOP position of the pie (above everything)
                # This creates a new pie segment above the existing Machine Tools content
                
                # TOP position - Cursor Alignment section (centered and above)
                box = pie.split()
                ...
                # Add the remaining empty pie positions to maintain layout
                pie.separator()  # RIGHT
